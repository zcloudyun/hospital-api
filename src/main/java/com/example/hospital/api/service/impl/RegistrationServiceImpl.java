...
public class RegistrationServiceImpl implements RegistrationService {
    ...
    public HashMap registerMedicalAppointment(Map param){
        ...
        try{
            ...
            int amount=MapUtil.getInt(param,"amount");
           ...
           //支付单的预支付ID
            ...
            //更新出诊计划实际挂号人数
            //更新数据库中该时段挂号人数和该医生当日挂号实际人数
             doctorWorkPlanDao.updateNumById(new HashMap(){{
                            put("id",workPlanId);
                            put("n",1);
             }});
            //在Redis中缓存付款记录，并设置过期时间
            //如果15分钟内患者支付了挂号费，会有java程序删除该缓存
            //如果该缓存过期，说明15分钟内患者没有支付挂号费
            ...
            HashMap result=new HashMap(){{
                ...
                put("timeStamp",date);
                put("amount",decimalFormat.format(Double.valueOf(temp)));
            }};
            return result;
        }catch (Exception e){
            if(redisTemplate.hasKey(key)){
    ...
    public boolean updatePayment(Map param){
        String outTradNo=MapUtil.getStr(param,"outTradeNo");
        //既然患者支付了挂号费，我们就删除Redis中对应的挂号缓存
        redisTemplate.delete("registration_payment_"+outTradNo);
        //更新挂号单付款为已付款状态（2状态）
        boolean bool=medicalRegistrationDao.updatePayment(param);
        return bool;
    }
    ...
    public boolean searchPaymentResult(String outTradeNo){
        String transactionId=medicalRegistrationDao.searchPaymentResult(outTradeNo);
    ...
    public Boolean updateByRegistrationId(Integer registrationId){
        return medicalRegistrationDao.updateByRegistrationId(registrationId);
    }
}
